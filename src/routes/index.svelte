...
<div class="map">
	<svg viewBox="0 0 60 60">
		{#each map as { x, y, type, building }}
			<g class="tile-group" class:active={isActive(x, y)} on:click={() => handleTileClick(y, x)}>
				<title>
					({y + 1}, {x + 1}) - {type || 'inconnu'}
					{#if building}{` [${building.type}]`}{/if}
				</title>
				<rect class={`tile tile-${type || 'inconnu'}`} {x} {y} width="1" height="1" />
				{#if building}
					<polygon
						class={`building building-${building.type || 'inconnu'}`}
						points="{x + 0.5},{y} {x + 1},{y + 0.5} {x + 1},{y + 1} {x},{y + 1} {x},{y + 0.5}"
					/>
				{/if}
				<circle class="dot" cx={x + 0.5} cy={y + 0.5} r="0.25" />
			</g>
		{/each}
	</svg>
</div>
